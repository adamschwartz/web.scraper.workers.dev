...
<html lang="en" with-selection-styled>
  <head>
    ...
    <script>/* dialog */(()=>{const a=a=>b=>{b.target.matches('input[type=text],input[type=email],input[type=number],input[type=password],input[type=search],input[type=tel],input[type=url],textarea')||"Escape"===b.key&&a.close()},b=a=>b=>{if(!a.contains(b.target)){const c=a.querySelectorAll('a[href]:not([tabindex="-1"]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),button:not([disabled])');a.previousElementSibling===b.target?c[c.length-1].focus():c[0].focus()}};window.Dialog=class{constructor(c){this.el=c,this.backdropEl=c.closest("[dialog-backdrop]"),this.trapFocus=b(c),this.checkForEscape=a(this),this.setupClosers(),this.setupSubmitters(),this.setupTransitioning()}setupClosers(){const a="[dialog-close]";this.backdropEl.addEventListener("click",b=>{const c=b.target;(c===this.backdropEl||c.matches(a)||c.closest(a))&&this.close()})}setupSubmitters(){const a=document.querySelector("[dialog-submit]");a&&a.addEventListener("submit",a=>{a.preventDefault(),this.close()})}setupTransitioning(){"all 0s ease 0s"===getComputedStyle(this.backdropEl).transition&&(this.backdropEl.style.transition="opacity 50ms ease"),this.backdropEl.addEventListener("transitionend",a=>{a.target!==this.backdropEl||document.documentElement.removeAttribute("is-dialog-transitioning")})}transition(){document.documentElement.setAttribute("is-dialog-transitioning","")}open(){this.originalActiveElement=document.activeElement,this.transition(),this.backdropEl.setAttribute("is-open",""),requestAnimationFrame(()=>{requestAnimationFrame(()=>{document.documentElement.setAttribute("is-dialog",""),document.addEventListener("focus",this.trapFocus,!0),document.addEventListener("keyup",this.checkForEscape,!0),this.el.querySelector("[dialog-autofocus]").focus()})})}close(){this.transition(),this.backdropEl.removeAttribute("is-open"),document.documentElement.removeAttribute("is-dialog"),document.removeEventListener("focus",this.trapFocus,!0),document.removeEventListener("keyup",this.checkForEscape,!0),this.originalActiveElement&&this.originalActiveElement.focus(),this.el.dispatchEvent(new CustomEvent("dialogClose",{detail:{dialog:this}}))}}})()</script>
